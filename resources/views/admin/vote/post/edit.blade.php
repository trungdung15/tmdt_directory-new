...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12">
            <!-- BEGIN: Form Layout -->
            <form action="{{route('vote.updatePost',['id'=>$vote->id])}}" method="post" id="form-post">
                <div class="intro-y box p-5">
                    <div class="mt-3">
                        <label for="crud-form-2" class="form-label">Bài viết</label>
                        <select name="post" data-placeholder="Chọn bài đăng" class="tom-select w-full change-post" data-vote="{{$vote->id}}" id="crud-form-2">
                            <option value="0">--Chọn bài viết</option>
                            @foreach($arrPost as $index => $item)
                                <option value="{{$index}}"
                                    {{ (old('post') == $index || (old('post') == null && $vote->post_id == $index)) ? 'selected': false }}
                                >{{$item}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mt-3">
                        ...
                        <select name="parent_id" data-placeholder="Chọn bình luận reply" class="form-select w-full select-reply" id="crud-form-2">
                            <option value="0">--Chọn bình luận--</option>
                            @foreach($arrVote as $index => $item)
                                <option value="{{$index}}"
                                    {{ (old('parent_id') == $index || (old('parent_id') == null && $vote->parent_id == $index)) ? 'selected': false }}
                                >{{$item}}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change','.change-post',function (e) {
                ...
                $.ajax({
                    url: "{{ route('vote.selectReplyEdit') }}",
                    method: "GET",
                    data: data,
                    dataType: "json",
                    beforeSend: function(){
